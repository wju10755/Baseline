...
#                                                                                                          #
############################################################################################################
# Check Bitlocker Compatibility -v
<#
...
# Function to check if Adobe Acrobat Reader is installed
function Is-AcrobatReaderInstalled {
    try {
        $acrobatKey = Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*" | Where-Object { $_.DisplayName -like "*Adobe Acrobat Reader*" }
        if (-not $acrobatKey) {
            throw "Adobe Acrobat Reader is not installed."
        }
        return $true
    } catch {
